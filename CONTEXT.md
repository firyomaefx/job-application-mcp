...
- Result: Immediate Windows failure class fixed locally and merged through PR #7 as `f22bdcd`; PR and authoritative-main CI run `31650948788` passed all eight jobs, including Linux/macOS/Windows package verification and the packaged Windows runtime smoke.
- Remaining work: A future CAPA-bearing release must use a new immutable version such as v0.4.3; do not retag v0.4.2. Installer signing remains required for production.
