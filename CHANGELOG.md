...
- **Portable `.exe` (run without install).** New `portable` Windows target
  (`desktop/package.json` `build.win.target`) produces a single self-extracting
  `Job Application MCP 0.4.1.exe` you can run from anywhere with no install.
- **One-click NSIS installer.** The `nsis` target is now `oneClick: true`
  (per-user, no install-directory wizard) — a single-click install. Replaces the
  earlier MSIX target (enterprise MSIX needs a code-signing cert; see gap below).
- **App icon.** A 512×512 indigo PNG (`desktop/icons/icon.png`, generated by the
  dependency-free `desktop/build-icon.js`) is now the build icon; electron-builder
  converts it to `.ico`. The NSIS installer shows this icon.
- **`signAndEditExecutable: false`.** The Windows build no longer attempts to
  sign/edit the executable, so it does **not** fetch the `winCodeSign` toolset
  (whose extraction needs Windows symlink privilege — `SeCreateSymbolicLinkPrivilege`
  / Developer Mode — that a normal user account doesn't have). This is correct
  anyway: we have no code-signing certificate, so the build is **unsigned**.
  - ⚠️ **Cosmetic side-effect:** because `rcedit` (which sets the `.exe` file icon
    and version metadata) is part of the skipped step, the **portable `.exe` and
    the installed app `.exe` keep Electron's default icon**. The **NSIS installer
    itself** uses our icon (NSIS applies `build.icon` independently). Fixing the
    `.exe` icon requires either a code-signing cert or running the build with
    symlink privilege (admin / Developer Mode) so `signAndEditExecutable` can be
    re-enabled — same gap already noted in `packaging/README.md`.
- Tool count unchanged (41); no schema or data changes; no new runtime egress.
